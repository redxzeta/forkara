...
describe("release update policy", () => {
  ...
  it("keeps clean release metadata on Latest and dedicated channel filenames", () => {
    const root = mkdtempSync(join(tmpdir(), "forkara-release-policy-"));
    try {
      ...
      expect(prepareReleaseUpdateManifests(root, cleanConfig)).toEqual([
        ...
        ...channelManifestNames("forkara"),
      ]);
      for (const name of defaultManifestNames) {
        expect(readFileSync(resolve(root, name), "utf8")).toBe(name);
      }
      for (const [index, channelName] of channelManifestNames("forkara").entries()) {
  ...
  it("refuses to overwrite a compatibility channel placeholder", () => {
    ...
    try {
      ...
      writeFileSync(resolve(root, "forkara-mac.yml"), "existing");

      expect(() => prepareReleaseUpdateManifests(root, { ...cleanConfig, lane: "bridge" })).toThrow(
        "Refusing to overwrite existing update manifest: forkara-mac.yml",
      );
      expect(existsSync(resolve(root, "forkara.yml"))).toBe(false);
      expect(existsSync(resolve(root, "forkara-linux.yml"))).toBe(false);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
