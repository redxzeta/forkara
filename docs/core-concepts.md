...
Forkara becomes much easier to use once its ownership model is clear: **each task owns one body of
...
| Concept          | Meaning                                                             |
| ---------------- | ------------------------------------------------------------------- |
| Workspace        | The complete Forkara application and the projects available in it   |
...
A project is the folder Forkara works with.
...
For work that should continue across several turns, set a deliberate thread goal. A goal can
continue after a clean turn, but queued user work, approvals, questions, interruptions, failures,
and pause rules remain in control.

Use a thread fork when a new task should inherit the conversation or split from one exact turn. Use
a handoff when another provider should continue the same task and ownership boundary.
...
Use separate Git worktrees before starting several tasks in the same repository.

## Providers, models, and sessions

A provider is the coding-agent runtime Forkara operates, such as Claude Code, Codex, OpenCode, Cursor,
...
Forkara supplies the shared workspace around it:
...
Each task owns a provider session. Available models and controls can differ because Forkara discovers
capabilities from the installed runtime and account.

## Handoffs

A provider handoff lets another supported provider continue the same task and working environment
using the context Forkara passes to it.
...
Forkara treats Git as the durable review and recovery layer.
...
Forkara's checkpoint and revert controls can help recover task work, but committed Git history remains
...
Use separate ownership boundaries and worktrees before scaling beyond one task.
...
The Settings keyboard-shortcuts panel contains the complete list for the installed build.
