...
Forkara keeps request accounting on each native query context. The current and
...
cannot distinguish them. Migration 100 preserves retained raw main-loop results
...
Migration 99 adds persisted message turn boundaries, and Claude accounting follows
at migration 100. Migration 101 adds asynchronous user input. Keep the
numeric `migrationEntries` values literal because the lineage checker parses
that source.

## Input and cache behavior

Forkara retains its existing harness policy and automation guidance. This selective
backport changes token accounting only; it makes no prompt-size or subscription-cost
claim.
...
tests, migration 100's upgrade test, context-window tests, and harness/tool
