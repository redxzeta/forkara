...
import {
  ForkaraCapabilitiesResult,
  ForkaraCreateThreadsInput,
  ForkaraCreateThreadsResult,
  ForkaraGatewayErrorResult,
  ForkaraWaitForThreadsInput,
  ForkaraWaitForThreadsResult,
} from "./agentGateway";

const decodeCreate = Schema.decodeUnknownSync(ForkaraCreateThreadsInput);
const decodeWait = Schema.decodeUnknownSync(ForkaraWaitForThreadsInput);
...
describe("agent gateway contracts", () => {
  ...
  it("decodes typed capability, creation, wait, and error results", () => {
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(ForkaraCapabilitiesResult)({
        targetConstruction: {
          codex: {
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(ForkaraCreateThreadsResult)({
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(ForkaraWaitForThreadsResult)({
    ...
    assert.doesNotThrow(() =>
      Schema.decodeUnknownSync(ForkaraGatewayErrorResult)({
