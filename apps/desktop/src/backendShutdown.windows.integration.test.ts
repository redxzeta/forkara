...
function makeIsolatedServerEnvironment(input: {
  ...
  return {
    ...environment,
    FORKARA_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "0",
    FORKARA_CLAUDE_KEEPALIVE: "0",
    FORKARA_DESKTOP_SHUTDOWN_TOKEN: input.shutdownToken,
    FORKARA_HOME: input.homeDir,
    FORKARA_HOST: "127.0.0.1",
    FORKARA_MODE: "desktop",
    FORKARA_NO_BROWSER: "1",
    FORKARA_PORT: String(input.port),
  };
}
...
describeWindows("Windows desktop backend shutdown integration", () => {
  ...
  it("authenticates a real server, deduplicates shutdown, drains its runtime, and clears its finalizer artifact", async () => {
    ...
    try {
      const homeDir = await resources.makeTempDir("forkara-windows-shutdown-");
