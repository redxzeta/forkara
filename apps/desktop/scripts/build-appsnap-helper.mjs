...
function buildFingerprint({ arch, release, sources, targets }) {
  ...
  hash.update("forkara-appsnap-helper-build-v1\0");
  hash.update(arch);
  hash.update("\0");
...
export function buildAppSnapHelper({
  ...
  const temporaryDirectory = mkdtempSync(join(tmpdir(), "forkara-appsnap-helper-"));
  ...
  try {
    ...
    for (const target of targets) {
      const thinBinary = join(temporaryDirectory, `forkara-appsnap-helper-${target.arch}`);
    ...
    const unsignedBinary = join(temporaryDirectory, "forkara-appsnap-helper");
