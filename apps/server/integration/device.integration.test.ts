/**
 * Device pane end-to-end verification against a live Forkara server.
 *
...
/**
 * Reboot the simulator behind Forkara's back, the way `simctl` from a terminal,
...
async function screenshotViaSimctl(udid: string): Promise<Buffer> {
  const path = join(tmpdir(), `forkara-device-e2e-${randomUUID()}.png`);
  try {
...
describeE2e("device pane end-to-end", () => {
  ...
  it("boots the target simulator", async () => {
    ...
    expect(booted?.bootSource).toBe("forkara");
  }, 180_000);
  ...
  it("keeps input working after the simulator is rebooted outside Forkara", async () => {
    if (!target) throw new Error("no target device");
