...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Forkara refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/forkara\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/forkara/", "refs/historical/");
