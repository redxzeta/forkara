...
import { FORKARA_GATEWAY_HARNESS_POLICY } from "./agentGateway/harnessPolicy.ts";
import {
...
Prefer Forkara's built-in browser for browser work. It may continue in the background without changing the user's active chat. In code mode, call its MCP methods directly inside \`functions.exec\` with the exact \`tools.mcp__forkara__browser_*\` prefix (for example, \`await tools.mcp__forkara__browser_open({ url })\` and \`await tools.mcp__forkara__browser_snapshot({})\`). The available suffixes are ${BROWSER_TOOL_NAMES.map((name) => `\`${name.slice("browser_".length)}\``).join(", ")}.
...
// Maps Forkara's simple runtime toggle to Codex thread-level permission overrides.
...
// Forkara re-sends turn-level Codex permission overrides, so keep "always allow"
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  private readonly forkaraSkillsDir: string | undefined;
  ...
  constructor(
    ...
    options?: {
      readonly forkaraSkillsDir?: string;
    ...
    this.forkaraSkillsDir = options?.forkaraSkillsDir;
  ...
  // The Forkara MCP server rides on the shared overlay config (no secrets),
  ...
  private async buildSessionProcessEnv(
    ...
    if (gatewayBearerToken) {
      env[FORKARA_AGENT_GATEWAY_TOKEN_ENV] = gatewayBearerToken;
    }
    return env;
  }

  // Registers `~/.forkara/skills` as a codex skill root so portable skills are
  ...
  private async registerForkaraSkillsRoot(context: CodexSessionContext): Promise<void> {
    if (!this.forkaraSkillsDir) {
      return;
    }
    try {
      await this.sendRequest(context, "skills/extraRoots/set", {
        extraRoots: [this.forkaraSkillsDir],
      });
    } catch (error) {
      // Older codex builds (< extra-roots support) keep working; Forkara-only
  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      await this.registerForkaraSkillsRoot(context);
      ...
      if (
        ...
        this.emitLifecycleEvent(
          ...
          "Starting a new Codex thread for a Forkara thread that previously had a provider binding.",
        );
        await Effect.logWarning(
          "codex app-server starting a fresh thread for a previously bound Forkara thread",
          {
            threadId,
  ...
  private async handleServerRequest(
    ...
    if (isUserInputRequest) {
      ...
      const detail = "Codex asked a question Forkara could not render, so it was declined.";
