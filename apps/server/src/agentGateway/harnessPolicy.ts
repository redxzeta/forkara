...
export const FORKARA_HARNESS_POLICY_VERSION = "2026-08-25.2";
export const FORKARA_HARNESS_POLICY_MARKER = `[Forkara harness policy ${FORKARA_HARNESS_POLICY_VERSION}]`;
 ...
 * Forkara resources.
 */
export function renderForkaraHarnessPolicy(capabilities: ForkaraHarnessCapabilities): string {
  const controlPolicy = capabilities.gatewayControlAvailable
    ? [
        "Use the forkara_* tools for Forkara threads, projects, automations, and coordination.",
        "Use the browser_* tools autonomously whenever the user refers in any language to Forkara's integrated, embedded, visible, or in-app browser. They are the canonical and complete control surface for that browser: do not load or use a generic Browser, Chrome, Computer Use, OS-automation, Node REPL, Playwright, or other browser-control skill/tool instead. They control the exact thread-scoped Electron page Forkara surfaces to the user, including its live DOM, cookies, and session. The page may continue in the background while the user views another chat; browser actions must never change the user's active chat. When no assigned tab exists, start with browser_open rather than browser_navigate. Take a fresh semantic browser_snapshot before element actions and after navigation or human interaction, requesting an image only when semantics are insufficient.",
        ...
        "For thread discovery and diagnosis, use forkara_list_threads, forkara_read_thread, forkara_read_thread_activity, forkara_read_thread_events, forkara_read_thread_runtime_events, and forkara_diagnose_thread before inspecting Forkara's SQLite files or process logs. Fall back to host storage only when a tool's coverage metadata says the required evidence is unavailable.",
        "Provider-native subagent or Task tools are implementation details: they do not create Forkara threads and must not substitute for an explicit request to create Forkara threads.",
        "For a plural thread request, submit one exact forkara_create_threads plan. The array length is the exact requested count.",
        "If forkara_create_threads rejects the plan during validation or preflight before returning an operationId, correct that same plan and retry it with the same requestId. This is safe because no durable operation, thread, or worktree was created.",
        "Use forkara_capabilities to select canonical provider, model, and option values. Never guess a model slug or silently substitute a provider or model.",
        "Provider option keys are not interchangeable: Codex uses options.reasoningEffort and Claude Agent uses options.effort. Follow forkara_capabilities.targetConstruction for every provider instead of inspecting Forkara source code.",
        "When results are requested, call forkara_wait_for_threads for the created thread ids, wait for every requested result, then synthesize all outcomes.",
        "After forkara_create_threads returns an operationId, retries must keep the same requestId and exact plan. Report terminal operation failures as outcomes; do not create replacement threads unless the user gives a new instruction.",
        "Forkara automations support heartbeat, standalone, and dedicated modes plus interval, once, daily, weekdays, weekly, and cron schedules. Existing everyMinutes heartbeat calls remain supported. Use fastInterval: true only when the user explicitly accepts a sub-minute bounded loop.",
        ...
        'Mode does not restrict stop conditions. completionPolicy {"type":"ai-evaluated","stopWhen":"..."} works in both modes and disables the automation when the clause matches a successful run; prefer it over encoding the stop condition in the prompt. maxIterations remains the backstop, and an automation-dispatched run may always call forkara_cancel_automation on its own automation.',
        AUTOMATION_AUTHORING_GUIDANCE,
        "Prefer forkara_create_automation with suggested: true when the user has not explicitly asked to create an automation. Suggested automations remain disabled until the user accepts their proposal card.",
        "Before forkara_update_automation, call forkara_view_automation and resend the complete mutable configuration, including unchanged fields. Updates are full replacement and partial payloads are rejected.",
        ...
        'During an automation-dispatched turn, persist durable context with forkara_update_automation_memory {"memory": "..."} before finishing; memory is full replacement, DB-backed, and capped at 32 KiB.',
        'Every automation-dispatched turn must finish by calling forkara_report_automation_result. Use decision "silent" only for a successful run with nothing requiring user attention; otherwise use "notify" with a concise title and summary. Failures remain visible regardless of this decision or the automation notification policy. Never call this tool for a manual follow-up turn.',
      ]
    : [
        "Forkara MCP control is unavailable in this provider session. Do not claim that Forkara threads, projects, or automations were created or changed.",
        "Provider-native subagent or Task tools do not create Forkara threads. If the user explicitly requests Forkara resource management, explain that this session cannot perform it.",
      ];

  return [
    FORKARA_HARNESS_POLICY_MARKER,
    "You are running inside Forkara. Forkara is the host and harness for this session.",
...
export const FORKARA_GATEWAY_HARNESS_POLICY = renderForkaraHarnessPolicy({
  gatewayControlAvailable: true,
});

export const FORKARA_IDENTITY_ONLY_HARNESS_POLICY = renderForkaraHarnessPolicy({
  gatewayControlAvailable: false,
});

export interface ForkaraHarnessPolicyDeliveryState {
  harnessPolicyDelivered?: boolean | undefined;
}

const PROVIDERS_WITH_THREAD_SCOPED_FORKARA_MCP = new Set<ProviderKind>([
  "codex",
  "claudeAgent",
  "antigravity",
  "cursor",
 ...
 */
export function takeForkaraHarnessPolicyForProviderSession(
  state: ForkaraHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  return takeForkaraHarnessPolicyForSession(state, {
    gatewayControlAvailable: providerHasForkaraGatewayControl(input),
  });
}

export function takeForkaraHarnessPolicyTextPartForProviderSession(
  state: ForkaraHarnessPolicyDeliveryState,
  input: {
    readonly provider: ProviderKind;
    readonly scopedGatewayConnectionAvailable: boolean;
  ...
  const text = takeForkaraHarnessPolicyForProviderSession(state, input);
