import { ORCHESTRATION_WS_METHODS, WS_METHODS } from "@forkara/contracts";
...
describe("WsRequestAdmission", () => {
  it("keeps lightweight shell reads out of the expensive lane", () => {
    ...
    expect(classifyWsRequest(WS_METHODS.gitOriginalityMeter)).toBe("expensive-read");
    expect(classifyWsRequest(WS_METHODS.gitForkSpeedrun)).toBe("expensive-read");
