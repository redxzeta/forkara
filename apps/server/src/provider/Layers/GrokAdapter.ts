...
import { buildAcpForkaraMcpServers } from "../../agentGateway/mcpInjection.ts";
import {
...
export const takeGrokForkaraHarnessPolicyTextPart = (
...
const GROK_ACP_DEBUG_ENV = "FORKARA_GROK_ACP_DEBUG";
const FORKARA_GROK_ACP_DEBUG_ENV = "FORKARA_GROK_ACP_DEBUG";
...
// override with FORKARA_GROK_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const GROK_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "FORKARA_GROK_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const GROK_PLAN_MODE_PROMPT_PREFIX = [
  "Forkara requested Grok's native plan mode.",
...
const GROK_PLAN_GUARD_HOOK_CALLBACK_ID = "forkara-plan-guard";
...
export function buildGrokPromptMeta(interactionMode: ProviderInteractionMode): {
  ...
  // the provider state when Forkara sends the desired mode again.
...
export function resolveGrokPlanHookResponse(
  ...
  return {
    decision: "deny",
    systemMessage: `Forkara Plan mode blocks the mutating or unknown Grok tool "${toolName || "unknown"}".`,
  };
}
...
function isGrokAcpDebugEnabled(): boolean {
  return (
    ...
    process.env[FORKARA_GROK_ACP_DEBUG_ENV] === "1" ||
...
export function makeGrokAdapter(
  ...
  return Effect.gen(function* () {
    ...
    // present, every session gets the forkara_* MCP tools.
      ...
      withThreadLock(
        ...
        Effect.gen(function* () {
          ...
          const acp = yield* makeGrokAcpRuntime({
            ...
            clientInfo: { name: "Forkara", version: "0.0.0" },
                ...
                Effect.gen(function* () {
                  ...
                  if (planMarkdown !== undefined) {
                    ...
                    if (
                      ...
                      ctx.lastPlanFingerprint = planMarkdown;
                      // The extension response must reach Grok before Forkara cancels the
    ...
    const startGrokTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeGrokForkaraHarnessPolicyTextPart(
          ctx,
              ...
              Effect.gen(function* () {
                ...
                if (terminalPlanMarkdown !== undefined) {
                  ...
                  yield* offerRuntimeEvent(ctx.lifecycleGeneration, {
                    ...
                    raw: {
                      source: "acp.jsonrpc",
                      method: "forkara.grok.terminal-plan-response",
                      payload: result,
                    },
                  });
                }
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Grok ACP version does not advertise session/fork; Forkara will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Grok session has a turn in flight; Forkara will rebuild the fork from its retained transcript.",
          });
        }
        const forked = activeSource
          ...
          : yield* Effect.gen(function* () {
              ...
              const runtime = yield* makeGrokAcpRuntime({
                ...
                clientInfo: { name: "Forkara Fork", version: "0.0.0" },
                sessionMeta: GROK_SESSION_META,
              });
