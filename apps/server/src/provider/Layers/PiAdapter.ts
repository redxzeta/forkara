...
// a native clipboard module, so importing it during Forkara startup can bloat the
  ...
  if (isRecord(result) && result.isError === true) {
    const message = Array.isArray(result.content)
      ? result.content
          ...
          .join("\n")
      : "";
    throw new Error(message || "Forkara gateway tool failed.");
  }
  const content =
...
export async function buildPiAgentGatewayCustomTools(input: {
  ...
  if (tools.length === 0) {
    throw new Error("Forkara MCP returned an empty tool catalog.");
  }
 ...
 * before it crosses Forkara's trimmed-string RPC contract. A single malformed
...
const makePiAdapter = (options?: PiAdapterLiveOptions) =>
  Effect.gen(function* () {
    ...
    // Bridges the common Pi extension UI primitives onto Forkara's existing
    ...
    const makePiExtensionUIContext = (context: PiSessionContext): ExtensionUIContext => {
      ...
      const warnUnsupported = (method: string) => {
        ...
        offerRuntimeEvent({
          ...
          type: "runtime.warning",
          payload: {
            message: `Pi extension UI API '${method}' is not supported in Forkara yet.`,
            detail: { method },
          },
          raw: {
            source: "pi.sdk.event",
      ...
      const uiContext: ExtensionUIContext = {
        ...
        // chrome. Forkara has its own working header; neither belongs in the
        ...
        setTheme() {
          return { success: false, error: "Forkara does not expose Pi themes." };
        },
        getToolsExpanded() {
          return false;
        },
    ...
    const buildProviderText = (context: PiSessionContext, text: string) =>
      [
        takeForkaraHarnessPolicyForProviderSession(context, {
          provider: PROVIDER,
          ...
          ? yield* releaseAgentGatewaySessionLeaseOnInterrupt(
              ...
              Effect.catch((cause) =>
                Effect.sync(() => agentGatewaySessionLease?.release()).pipe(
                  Effect.andThen(
                    Effect.logWarning(
                      "Pi could not install thread-scoped Forkara gateway tools",
                      cause,
                    ),
                  ),
      ...
      dispatchLock.withLock(
        ...
        Effect.gen(function* () {
          ...
          if (context.runtime.session.isStreaming) {
            // A run Forkara did not dispatch is active (e.g. extension-triggered).
            return yield* sendTurnBusyError();
          }
    ...
    const respondUnsupported = (threadId: ThreadId, method: string) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method,
          detail: `Pi does not expose Forkara approval/user-input requests for thread ${threadId}.`,
        }),
      );
