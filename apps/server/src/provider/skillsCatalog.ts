...
//          walking) plus the unified cross-provider skills catalog backing Forkara
//          portable skills. Aggregates `~/.forkara/skills` with every provider-native
...
//          mergeSkillsIntoCatalog, filterDisabledSkills, ensureForkaraSkillsDir
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Forkara base dir (usually `~/.forkara`); skills live in `{base}/skills`. */
  readonly forkaraBaseDir: string;
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Forkara. */
  readonly includeForkaraRoot?: boolean;
}
...
const ensuredForkaraSkillsDirs = new Set<string>();

export function clearSkillsCatalogCacheForTests(): void {
  skillsCatalogCache.clear();
  skillsCatalogInflight.clear();
  ensuredForkaraSkillsDirs.clear();
}

export function forkaraSkillsDir(forkaraBaseDir: string): string {
  return nodePath.join(forkaraBaseDir, "skills");
}

// Creates the portable skills folder on first use so users have a drop-in target.
export async function ensureForkaraSkillsDir(forkaraBaseDir: string): Promise<string> {
  const dir = forkaraSkillsDir(forkaraBaseDir);
  if (ensuredForkaraSkillsDirs.has(dir)) {
    return dir;
  }
  try {
    await fs.mkdir(dir, { recursive: true });
    ensuredForkaraSkillsDirs.add(dir);
  } catch {
...
const SKILL_ORIGIN_ROOTS = {
  forkara: {
    homeRoots: (input) => [forkaraSkillsDir(input.forkaraBaseDir)],
    projectRootNames: [".forkara"],
  },
  codex: {
    // Keep Forkara's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Forkara as the
...
function orderedOriginsForProvider(
  provider: ProviderKind | null | undefined,
  includeForkaraRoot = true,
  includeRemainingOrigins = true,
): SkillsHomeOrigin[] {
  const preferred = preferredOriginsForProvider(provider);
  const ordered = [...preferred];
  if (includeForkaraRoot && !ordered.includes("forkara")) {
    ordered.push("forkara");
  }
  if (!includeRemainingOrigins) {
    return ordered.filter((origin) => includeForkaraRoot || origin !== "forkara");
  }
  for (const origin of HOME_ORIGIN_ORDER) {
    if (!includeForkaraRoot && origin === "forkara") {
      continue;
    }
...
export function skillsCatalogRoots(input: SkillsCatalogRootInput): SkillRoot[] {
  return rootsForOrderedOrigins(
    input,
    orderedOriginsForProvider(input.provider, input.includeForkaraRoot !== false),
  );
}
...
export async function discoverSkillsCatalog(
  ...
  const scan = (async () => {
    await ensureForkaraSkillsDir(input.forkaraBaseDir);
    const roots = [
