...
async function makeDbPath(): Promise<string> {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "forkara-sqlite-live-"));
  ...
  it("owns the live WAL exclusively without exposing a shared-memory sidecar", async () => {
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* sql`INSERT INTO ownership_probe(value) VALUES ('owned-by-forkara')`;
        ...
        `;
        expect(rows).toEqual([{ value: "owned-by-forkara" }]);
        yield* Effect.promise(async () => {
          await expect(fs.stat(`${dbPath}-shm`)).rejects.toMatchObject({ code: "ENOENT" });
        });
      }).pipe(
