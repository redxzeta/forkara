...
/**
 * A tracker identity that a *released* Forkara build wrote for a migration whose
 ...
 * does the migration Forkara registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Forkara's latest ID,
 * every Forkara migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Forkara's lineage and
 ...
 *
 * Divergences caused by Forkara renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Forkara build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Forkara migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Forkara build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Forkara lineage (legacy import); re-running migrations from the divergence point",
