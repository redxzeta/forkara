...
typedef union {
  ForkaraIndigoTouch touch;
  ForkaraIndigoButton button;
...
typedef struct {
  ...
  unsigned int field3;
  ForkaraIndigoEvent event;
} ForkaraIndigoPayload;

typedef struct {
  ForkaraIndigoMachHeader header;
  unsigned int innerSize;
  unsigned char eventType;
  ForkaraIndigoPayload payload;
} ForkaraIndigoMessage;
...
typedef ForkaraIndigoMessage *(*ForkaraIndigoButtonFn)(int keyCode, int op, int target);
typedef ForkaraIndigoMessage *(*ForkaraIndigoKeyboardFn)(uint32_t usage, int op);
typedef ForkaraIndigoMessage *(*ForkaraIndigoMouseFn)(CGPoint *point0, CGPoint *point1, int target,
                                                    int eventType, BOOL extra);
typedef ForkaraIndigoMessage *(*ForkaraIndigoArbitraryFn)(int target, uint32_t page, uint32_t usage,
                                                        int op);

BOOL ForkaraHardwareButtonFromName(NSString *name, ForkaraHardwareButton *outButton) {
  NSDictionary<NSString *, NSNumber *> *map = @{
    @"home": @(ForkaraHardwareButtonHome),
    @"lock": @(ForkaraHardwareButtonLock),
    @"side": @(ForkaraHardwareButtonSide),
    @"siri": @(ForkaraHardwareButtonSiri),
    @"volume-up": @(ForkaraHardwareButtonVolumeUp),
    @"volume-down": @(ForkaraHardwareButtonVolumeDown),
  };
  ...
  if (outButton != NULL) {
    *outButton = (ForkaraHardwareButton)found.integerValue;
  }
  return YES;
}

static int ForkaraButtonSource(ForkaraHardwareButton button) {
  switch (button) {
    case ForkaraHardwareButtonHome: return kButtonSourceHome;
    case ForkaraHardwareButtonLock: return kButtonSourceLock;
    case ForkaraHardwareButtonSide: return kButtonSourceSide;
    case ForkaraHardwareButtonSiri: return kButtonSourceSiri;
    case ForkaraHardwareButtonVolumeUp:
    case ForkaraHardwareButtonVolumeDown: return -1;
  }
...
static uint32_t ForkaraConsumerUsage(ForkaraHardwareButton button) {
  switch (button) {
    case ForkaraHardwareButtonVolumeUp: return kHIDUsageVolumeIncrement;
    case ForkaraHardwareButtonVolumeDown: return kHIDUsageVolumeDecrement;
    default: return 0;
  }
}
...
static uint32_t ForkaraUsageForCharacter(unichar c, BOOL *outShift) {
  BOOL shift = NO;
  uint32_t usage = 0;
...
@implementation ForkaraHIDBridge {
  id _client;
  dispatch_queue_t _sendQueue;
  ForkaraIndigoButtonFn _buttonFn;
  ForkaraIndigoKeyboardFn _keyboardFn;
  ForkaraIndigoMouseFn _mouseFn;
  ForkaraIndigoArbitraryFn _arbitraryFn;
...
- (BOOL)attachToDevice:(id)device error:(NSError **)error {
  NSString *developerDir = NSProcessInfo.processInfo.environment[@"FORKARA_DEVELOPER_DIR"];
  ...
  if (kit == NULL) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.forkara.device-helper.hid"
                                   code:1
  ...
  _arbitraryFn = (ForkaraIndigoArbitraryFn)dlsym(kit, "IndigoHIDMessageForHIDArbitrary");
  _buttonFn = (ForkaraIndigoButtonFn)dlsym(kit, "IndigoHIDMessageForButton");
  _keyboardFn = (ForkaraIndigoKeyboardFn)dlsym(kit, "IndigoHIDMessageForKeyboardArbitrary");
  _mouseFn = (ForkaraIndigoMouseFn)dlsym(kit, "IndigoHIDMessageForMouseNSEvent");
  ...
  if (client == nil) {
    if (error) {
      *error = initError ?: [NSError errorWithDomain:@"dev.forkara.device-helper.hid"
                                                code:4
  ...
  _client = client;
  _sendQueue = dispatch_queue_create("dev.forkara.device-helper.hid", DISPATCH_QUEUE_SERIAL);
  return YES;
}

- (void)sendMessage:(ForkaraIndigoMessage *)message {
  ...
  ((void (*)(id, SEL, ForkaraIndigoMessage *, BOOL, dispatch_queue_t, void (^)(NSError *)))objc_msgSend)(
...
- (void)sendTouchAtX:(double)x y:(double)y down:(BOOL)down {
  ...
  ForkaraIndigoMessage *seed = _mouseFn(&point, NULL, kTouchTarget, down ? kButtonOpDown : kButtonOpUp, NO);
  if (seed == NULL) {
    [self noteUndelivered];
    return;
  }
  ...
  size_t stride = sizeof(ForkaraIndigoPayload);
  ForkaraIndigoMessage *message = calloc(1, sizeof(ForkaraIndigoMessage) + stride);
  if (message == NULL) {
    free(seed);
    [self noteUndelivered];
    return;
  ...
  memcpy(&message->payload.event.touch, &seed->payload.event.touch, sizeof(ForkaraIndigoTouch));
  ...
  ForkaraIndigoPayload *secondPayload = (ForkaraIndigoPayload *)second;
...
- (void)sendKeyUsage:(uint32_t)usage down:(BOOL)down {
  ...
  ForkaraIndigoMessage *message = _keyboardFn(usage, down ? kButtonOpDown : kButtonOpUp);
  [self sendMessage:message];
}
...
- (NSInteger)typeText:(NSString *)text {
  ...
  for (NSUInteger index = 0; index < length; index++) {
    ...
    BOOL needsShift = NO;
    uint32_t usage = ForkaraUsageForCharacter(character, &needsShift);
    if (usage == 0) {
      skipped++;
      continue;
    }
...
- (void)sendButton:(ForkaraHardwareButton)button down:(BOOL)down {
  ...
  uint32_t consumerUsage = ForkaraConsumerUsage(button);
  ...
  [self sendMessage:_buttonFn(ForkaraButtonSource(button), op, kButtonTargetHardware)];
}

- (void)tapButton:(ForkaraHardwareButton)button {
