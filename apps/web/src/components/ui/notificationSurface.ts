...
export const COMPACT_NOTIFICATION_SURFACE_CLASS_NAME = `w-max max-w-[min(calc(100vw-2rem),28rem)] rounded-xl border border-[color-mix(in_srgb,var(--color-text-accent)_24%,transparent)] bg-[color-mix(in_srgb,var(--color-text-accent)_22%,transparent)] ${NOTIFICATION_FOREGROUND_CLASS_NAME} shadow-lg/15 backdrop-blur-xl before:hidden [-webkit-app-region:no-drag] dark:border-[color-mix(in_srgb,var(--color-text-accent)_10%,transparent)] dark:bg-[color-mix(in_srgb,var(--color-text-accent)_10%,transparent)]`;

export const EXPANDED_NOTIFICATION_SURFACE_CLASS_NAME = `w-full rounded-2xl border border-[color-mix(in_srgb,var(--color-text-accent)_24%,transparent)] bg-[color-mix(in_srgb,var(--color-text-accent)_22%,transparent)] ${NOTIFICATION_FOREGROUND_CLASS_NAME} shadow-lg/15 backdrop-blur-xl before:hidden [-webkit-app-region:no-drag] dark:border-[color-mix(in_srgb,var(--color-text-accent)_10%,transparent)] dark:bg-[color-mix(in_srgb,var(--color-text-accent)_10%,transparent)]`;
