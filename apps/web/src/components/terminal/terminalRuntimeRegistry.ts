...
// Purpose: Keep one xterm runtime alive per terminal id and attach/detach it from pane hosts.
// Layer: Terminal runtime infrastructure
// Depends on: xterm addons plus the native terminal bridge.
...
function clearDomDisposables(entry: TerminalRuntimeEntry): void {
...
function syncRuntimeConfig(entry: TerminalRuntimeEntry, config: TerminalRuntimeConfig): void {
...
function createRuntimeEntry(config: TerminalRuntimeConfig): TerminalRuntimeEntry {
...
async function openTerminal(entry: TerminalRuntimeEntry): Promise<void> {
  const api = readNativeApi();
...
function updateViewState(entry: TerminalRuntimeEntry, nextViewState: TerminalRuntimeViewState): void {
...
class TerminalRuntimeRegistry {
  ...
  attach(config: TerminalRuntimeConfig, viewState: TerminalRuntimeViewState, container: HTMLDivElement) {
    ...
    if (entry.container !== container) {
      this.detach(config.runtimeKey);
      entry.container = container;
      container.append(entry.wrapper);
    }

    updateViewState(entry, viewState);
    ensureResizeObserver(entry);
    startVisibilityRecovery(entry);
    void openTerminal(entry);
  ...
  getTerminal(runtimeKey: string): Terminal | null {
    return this.entries.get(runtimeKey)?.terminal ?? null;
  }

  getSearchAddon(runtimeKey: string): SearchAddon | null {
    return this.entries.get(runtimeKey)?.searchAddon ?? null;
  }
