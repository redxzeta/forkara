...
const pipePath = process.env.FORKARA_BROWSER_HOST_PIPE_PATH;
const capability = process.env.FORKARA_BROWSER_HOST_CAPABILITY;
const shellPath = process.env.FORKARA_E2E_SHELL_PATH;
const threadId = process.env.FORKARA_E2E_THREAD_ID as ThreadId | undefined;
const forkaraHome = process.env.FORKARA_HOME;
const annotationPreloadPath = process.env.FORKARA_E2E_BROWSER_ANNOTATION_PRELOAD;

if (!pipePath || !capability || !shellPath || !threadId || !forkaraHome || !annotationPreloadPath) {
  throw new Error("The visible-browser Electron fixture requires its isolated E2E environment.");
}

app.setPath("userData", path.join(forkaraHome, "electron-userdata"));
...
function setPanelVisible(visible: boolean): void {
  ...
  pushState();
  mainWindow?.webContents.send("forkara-e2e:open-panel");
}
function pushState(): void {
  if (shellReady && latestState && mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("forkara-e2e:browser-state", latestState);
  }
}
...
ipcMain.on("forkara-e2e:shell-ready", () => {
  shellReady = true;
  pushState();
});

ipcMain.handle(
  "forkara-e2e:attach-webview",
